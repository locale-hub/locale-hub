...
const renderEntry = (entry: DataEntry, idx: number) => {
  const classes = `flex items-center p-2 text-base font-normal ${
    colors[entry.type ?? 'default']
  ...
  if (undefined !== entry.onClick) {
    return (
      <a onClick={entry.onClick} key={idx} className={classes}>
        {entry.icon}
        <span className="ml-3">{entry.name}</span>
      </a>
    );
...
export default function Sidebar({ data }: { data: DataEntry[] }) {
  return (
    <aside className="w-full h-full" aria-label="Sidebar">
      <div className="overflow-y-auto py-4 px-3  h-full border-r border-slate-400/50">
        <ul className="space-y-2">{data.map(renderEntry)}</ul>
      </div>
    </aside>
  );
}
