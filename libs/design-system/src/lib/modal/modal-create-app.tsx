...
export default function ModalCreateApp({
  ...
  const [app, setApp] = useState<{ name: string, identifier: string }>({ name: '', identifier: '' })
  ...
  return <>
    ...
    <Dialog as="div" className="relative z-50 " open={isOpen} onClose={() => onClose(app)}>
        ...
        <div className="flex min-h-full items-center justify-center p-4 text-center">
            <Dialog.Panel className="max-w-2xl transform overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 text-left align-middle shadow-xl transition-all">
              ...
              <div className="mt-2 text-black">
                <div className='w-96'>
                  <InputField name={'name'} label={'Name'} onValue={setAppName} type={'text'} value={app.name} placeholder='Name' />
                  <InputField name={'identifier'} label={'Identifier'} onValue={setAppId} type={'text'} value={app.identifier} placeholder='Identifier' />
                </div>
              </div>

              <div className="mt-8 text-black text-right">
                <Button type='cancel' onClick={() => onClose()}>Cancel</Button>
                <Button type='confirm' onClick={() => onClose(app)}>Create</Button>
              </div>
            </Dialog.Panel>
        </div>
      </div>
