...
export default function ProjectUsersPage({
  ...
  useEffect(() => {
    ...
    ApiConnector.projects.get(params.projectId).then(data => {
      ...
      ApiConnector.organizations.users.list(data.project.organizationId).then(data2 => {
  ...
  return <>
    ...
    <AddUserModal isOpen={inviteUserModalOpen} onClose={addUser}
        users={organizationUsers.filter(ou => false === users.map(pu => pu.id).includes(ou.id))}
    />
