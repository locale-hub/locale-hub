...
export default function AddLocaleModal({
  ...
  return <Dialog as="div" className="relative" open={isOpen} onClose={() => onClose(locale)}>
      ...
      <div className="flex min-h-full items-center justify-center p-4 text-center">
          <Dialog.Panel className="w-2xl h-2xl transform rounded-2xl bg-white dark:bg-slate-800 p-6 text-left align-middle shadow-xl transition-all">
            ...
            <div className="mt-2 text-black">
              <div className='w-96'>
                <Select label='Locale'
                  ...
                  defaultSelected={{ id: locales[0].tag, value: `${locales[0].tag} - ${locales[0].name}` }}
                  values={locales.map(locale => ({ id: locale.tag, value: `${locale.tag} - ${locale.name}` }))}
                />
              </div>
            </div>
            ...
            <div className="mt-8 text-black text-right">
              ...
              <Button type='confirm' onClick={() => onClose(locale)}>Save</Button>
            </div>
          </Dialog.Panel>
      </div>
    </div>
