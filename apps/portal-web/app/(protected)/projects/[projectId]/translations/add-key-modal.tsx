...
export default function AddKeyModal({
  ...
  return <Dialog as="div" className="relative" open={isOpen} onClose={() => onClose(key)}>
      ...
      <div className="flex min-h-full items-center justify-center p-4 text-center">
          <Dialog.Panel className="w-2xl h-2xl transform rounded-2xl bg-white dark:bg-slate-800 p-6 text-left align-middle shadow-xl transition-all">
            ...
            <div className="mt-2 text-black">
              <div className='w-96'>
                <InputField name={'key'} label={'Key'} onValue={setKey} type={'text'} value={key} placeholder='Translation key' />
              </div>
            </div>

            <div className="mt-8 text-black text-right">
              <Button type='cancel' onClick={() => onClose()}>Cancel</Button>
              <Button type='confirm' onClick={() => onClose(key)}>Add</Button>
            </div>
          </Dialog.Panel>
      </div>
    </div>
