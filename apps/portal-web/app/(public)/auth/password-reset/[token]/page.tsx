...
export default function PasswordResetApplyPage({
  ...
  const formInvalid = () =>
    'error' in schema.validate({ email, password, passwordConfirm });

  const doPasswordReset = () => {
    ApiConnector.auth
      .resetPasswordApply(params.token, email, password)
      .then((data) => {
        if ('error' in data) {
  ...
  return (
    <div className="flex flex-col px-10 py-10 w-1/4 items-center m-auto justify-center">
      <h2 className="mt-6 text-3xl font-bold">Password reset</h2>
