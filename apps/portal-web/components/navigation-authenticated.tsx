    ...
    <>
      {0 !== organizations.length && (
        <AddProjectModal
          ...
          onClose={onProjectAdd}
          organizations={organizations}
        />
      )}
      <AddOrganizationModal
        isOpen={openAddOrganization}
        onClose={onOrganizationAdd}
      />
      <SlideOver
      ...
      >
        {notifications
          .filter((n) => NotificationStatus.UNREAD === n.status)
          .map((notification) => (
            <NotificationCard
              key={notification.id}
      ...
      <Link href={routes.organizations.root} className="px-4">
        Organizations
      </Link>
      <Link href={routes.projects.root} className="px-4">
        Projects
      </Link>
      <Spacer />
      ...
      >
        <div className="inline-flex absolute -top-2 right-0 justify-center items-center w-5 h-5 text-xs font-bold rounded-full bg-primary">
          {
            notifications.filter((n) => NotificationStatus.UNREAD === n.status)
              .length
          }
        </div>
      ...
      <Menu as="div" className={`relative`}>
        ...
        <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md rounded-md py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <a
            ...
            className="bg-gray-100 block px-4 py-2 text-black hover:cursor-pointer hover:bg-slate-200"
          >
            Project
          </a>
          <a
      ...
      <UserIcon name={user?.name} className="ml-3">
        ...
        <a
          onClick={logout}
          className="bg-gray-100 block px-4 py-2 text-warn hover:cursor-pointer hover:bg-slate-200"
        >
          Logout
        </a>
      </UserIcon>
