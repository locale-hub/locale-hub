 ...
 */
router.get('/:commitId', async function (req: Request, res: Response) {
  try {
...
router.post('/', async function (req: Request, res: Response) {
  try {
    ...
    await updateManifestFromProject(
      userId,
      projectId,
      changeList,
      commitTitle,
 ...
 */
router.put('/:commitId', async function (req: Request, res: Response) {
  try {
    const projectId = req.params.projectId;
    const commitId = req.params.commitId;
    ...
    const success = await CommitService.publish(
      projectId,
      commitId,
      body.deployed
    );
    ...
    await sdkUpdatePublishedCommit(
      projectId,
      body.deployed ? commit : undefined
    );
